import "../styles/policies.css";

export default function RefundPolicy() {
  return (
    <div className="policy-container">
      <h1>سياسة الاسترجاع</h1>
      <p><strong>آخر تحديث: 08-12-2025</strong></p>

      <p>نظرًا لأن المحتوى رقمي ويتم الوصول إليه فور الشراء، فإن سياسة الاسترجاع لدينا هي:</p>

      <h3>1. عدم إمكانية الاسترجاع</h3>
      <p>بعد الدفع ومنح الوصول، لا يمكن استرداد المبلغ.</p>

      <h3>2. استثناءات</h3>
      <ul>
        <li>عدم حصول المستخدم على المحتوى بعد الدفع</li>
        <li>وجود مشكلة تقنية من جانبنا</li>
      </ul>

      <h3>4. تواصل معنا</h3>
      <p>talentedplatform@gmail.com</p>
    </div>
  );
}
